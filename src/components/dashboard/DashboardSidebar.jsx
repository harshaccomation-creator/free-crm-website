...
const routes = {
  ...
  admin: { Dashboard: '/admin/dashboard', Leads: '/leads', Deals: '/admin/dashboard', Contacts: '/admin/dashboard', Companies: '/admin/dashboard', Tasks: '/admin/dashboard', Activities: '/admin/dashboard', Reports: '/admin/dashboard', Calendar: '/admin/dashboard', Users: '/admin/dashboard', Settings: '/admin/dashboard' },
...
export default function DashboardSidebar({ role = 'employee' }) {
  ...
  useEffect(() => {
    const nav = sidebarRef.current?.querySelector('.sfx-nav');
  ...
  return (
    <aside className="sfx-sidebar" ref={sidebarRef}>
      <div className="sfx-brand-row">
        <button className="sfx-brand sfx-brand-image sfx-brand-dark" onClick={() => navigateTo(homePath)} type="button"><SidebarBrand /></button>
      ...
      <nav className="sfx-nav">
        {items.map((item, index) => {
          ...
          return <button key={item} type="button" className={isActive(item, path, currentPath, index) ? 'active' : ''} onClick={() => navigateTo(path)}><span className="sfx-nav-icon"><Icon name={iconByItem[item]} /></span><em>{item}</em></button>;
        })}
      </nav>
