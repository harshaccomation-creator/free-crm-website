...
const menuByRole = {
  ...
  superAdmin: ['Dashboard', 'Users', 'Roles & Permissions', 'Organizations', 'Modules', 'Plans & Billing', 'Settings', 'Activity Logs', 'System Logs', 'Integrations', 'Backup'],
};
...
function isActive(item, path, currentPath, index) { if (path) return currentPath === path || ((item === 'My Leads' || item === 'Team Leads') && currentPath.startsWith('/leads')); return index === 0 && currentPath.includes('/dashboard'); }
...
export default function DashboardSidebar({ role = 'employee' }) {
  ...
  const label = safeRole === 'superAdmin' ? 'Enterprise Control' : safeRole === 'admin' ? 'Company Admin' : safeRole === 'manager' ? 'Manager Workspace' : 'Premium CRM';
  useEffect(() => { const nav = sidebarRef.current?.querySelector('.sfx-nav'); if (nav && nav.scrollTop < 4) nav.scrollTop = 0; });
  return <aside className="sfx-sidebar" ref={sidebarRef}><div className="sfx-brand-row"><button className="sfx-brand sfx-brand-image sfx-brand-dark" onClick={() => navigateTo(homePath)} type="button"><SidebarBrand /></button><button className="sfx-menu" type="button" aria-label="Toggle menu"><Icon name="menu" /></button></div><nav className="sfx-nav">{items.map((item, index) => { const path = routes[safeRole]?.[item]; return <button key={item} type="button" className={isActive(item, path, currentPath, index) ? 'active' : ''} onClick={() => navigateTo(path)}><span className="sfx-nav-icon"><Icon name={iconByItem[item]} /></span><em>{item}</em></button>; })}</nav><div className="sfx-upgrade"><span>*</span><h3>{label}</h3><p>Clean CRM workspace with role based navigation.</p><button type="button" onClick={() => navigateTo('/settings')}>Open Settings</button></div><button className="sfx-help" type="button" onClick={() => navigateTo('/notifications')}><span>?</span><div><strong>CRM Alerts</strong><small>Open Notifications</small></div></button></aside>;
}
