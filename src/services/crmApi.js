...
function toDbLeadStatus(nextStatus) {
  ...
  if (nextStatus === 'Lost' || nextStatus === 'Junk') return 'Lost';
  if (nextStatus === 'Contacted' || nextStatus === 'Not Connected') return 'Contacted';
  if (nextStatus === 'Demo Scheduled' || nextStatus === 'Demo Done' || nextStatus === 'Follow-up') return 'Qualified';
  return null;
}
...
async function enrichLeadsWithActivityStatus(client, leads = []) {
  ...
  (data || []).forEach((activity) => { if (!activitiesByLead.has(activity.lead_id)) activitiesByLead.set(activity.lead_id, []); activitiesByLead.get(activity.lead_id).push(activity); if (statusByLead.has(activity.lead_id)) return; const next = getActivityLeadStatus(activity); if (next) statusByLead.set(activity.lead_id, next); });
  return leads.map((lead) => { const activities = activitiesByLead.get(lead.id) || []; return { ...lead, score: calculateLeadScoreFromActivities(activities), status: statusByLead.has(lead.id) ? statusByLead.get(lead.id) : lead.status }; });
}
