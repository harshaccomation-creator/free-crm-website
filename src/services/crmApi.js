...
export async function getCompanyUsers(companyId) {
  const client = requireBackend();
  const { data, error } = await client.from('profiles').select('id, full_name, email, role, phone, is_active').eq('company_id', companyId).eq('is_active', true).order('full_name', { ascending: true });
...
export async function getSubscription(companyId) {
  const client = requireBackend();
  const { data, error } = await client.from('companies').select('id, plan, plan_status, trial_start_at, trial_ends_at').eq('id', companyId).maybeSingle();
  if (error && error.code !== 'PGRST116') handleError(error, 'Unable to load subscription');
  return data ? { company_id: data.id, plan_name: data.plan || 'trial', status: data.plan_status || 'trial', trial_started_at: data.trial_start_at, trial_ends_at: data.trial_ends_at } : null;
}

export function isSubscriptionActive(subscription) {
  if (!subscription) return false;
  if (subscription.status === 'active') return true;
  if (subscription.status === 'trial') return !subscription.trial_ends_at || new Date(subscription.trial_ends_at).getTime() >= Date.now();
  return false;
}
...
export async function listLeads({ status, assignedToMe = false, search = '', limit = 100 } = {}) {
  ...
  let query = client.from('leads').select('*, owner:profiles!leads_owner_id_fkey(id, full_name, email, role)').eq('company_id', profile.company_id).eq('is_deleted', false).order('created_at', { ascending: false }).limit(limit);
  if (assignedToMe || isEmployee(profile)) query = query.eq('owner_id', profile.id);
...
export async function getLead(leadId) {
  ...
  let query = client.from('leads').select('*, owner:profiles!leads_owner_id_fkey(id, full_name, email, role)').eq('id', leadId).eq('company_id', profile.company_id).maybeSingle();
...
export async function listTasks({ status, from, to, assignedToMe = false, limit = 100 } = {}) {
  ...
  const profile = await getCurrentProfile();
  if (!profile?.company_id) return [];
  let query = client.from('tasks').select('*, lead:leads(id, name, company, phone), owner:profiles!tasks_owner_id_fkey(id, full_name, email)').eq('company_id', profile.company_id).order('due_at', { ascending: true }).limit(limit);
  if (assignedToMe || isEmployee(profile)) query = query.eq('owner_id', profile.id);
  if (status) query = query.eq('status', status);
...
export async function createTask(payload) {
  ...
  const record = { company_id: profile.company_id, created_by: profile.id, owner_id: payload.owner_id || payload.assigned_to || profile.id, lead_id: payload.lead_id || null, title: String(payload.title || '').trim(), note: payload.note || payload.description || null, type: payload.type || payload.task_type || 'Call', status: payload.status || 'Pending', due_at: payload.due_at || null };
  if (!record.title) throw new Error('Task title is required.');
  const { data, error } = await client.from('tasks').insert(record).select('*, lead:leads(id, name, company, phone)').single();
  handleError(error, 'Unable to create task');
  if (data.lead_id) { try { await createActivity({ lead_id: data.lead_id, type: 'task_created', title: 'Task created', note: data.title }); } catch {} }
  return data;
}
...
export async function updateTask(taskId, payload) {
  ...
  let query = client.from('tasks').update(patch).eq('id', taskId).eq('company_id', profile.company_id);
  if (isEmployee(profile)) query = query.eq('owner_id', profile.id);
  const { data, error } = await query.select('*, lead:leads(id, name, company, phone)').single();
  handleError(error, 'Unable to update task');
  return data;
}

export async function listActivities({ leadId, limit = 100, assignedToMe = false } = {}) {
  const client = requireBackend();
  const profile = await getCurrentProfile();
  if (!profile?.company_id) return [];
  let query = client.from('lead_activities').select('*, lead:leads!inner(id, name, owner_id, company), user:profiles!lead_activities_user_id_fkey(id, full_name, email)').eq('company_id', profile.company_id).order('activity_at', { ascending: false }).limit(limit);
  if (leadId) query = query.eq('lead_id', leadId);
  if (assignedToMe || isEmployee(profile)) query = query.eq('lead.owner_id', profile.id);
  const { data, error } = await query;
...
export async function listMyNotifications({ limit = 20 } = {}) {
  ...
  const { data, error } = await client.from('notifications').select('*').eq('user_id', profile.id).order('created_at', { ascending: false }).limit(limit);
  if (error) { console.warn('[SalesFlow CRM API] notifications unavailable', error); return []; }
  return data || [];
}
...
export async function markNotificationRead(id) {
  const client = requireBackend();
  const profile = await getCurrentProfile();
  const { data, error } = await client.from('notifications').update({ read_at: new Date().toISOString() }).eq('id', id).eq('user_id', profile.id).select('*').single();
  handleError(error, 'Unable to mark notification read');
  return data;
}
...
export async function markAllNotificationsRead() {
  const client = requireBackend();
  const profile = await getCurrentProfile();
  const { error } = await client.from('notifications').update({ read_at: new Date().toISOString() }).eq('user_id', profile.id).is('read_at', null);
  handleError(error, 'Unable to mark notifications read');
  return true;
}
...
export async function createNotification(payload) {
  const client = requireBackend();
  const profile = await getCurrentProfile();
  const record = { company_id: profile.company_id, user_id: payload.user_id || profile.id, title: payload.title, message: payload.message || null, type: payload.type || 'info', related_lead_id: payload.related_lead_id || payload.lead_id || null };
...
export async function getMyDashboardSummary() {
  const [profile, leads, tasks, activities, notifications] = await Promise.all([getCurrentProfile(), listMyLeads({ limit: 1000 }), listMyTasks({ limit: 1000 }), listMyActivities({ limit: 200 }), listMyNotifications({ limit: 50 })]);
  const now = new Date();
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const tomorrow = new Date(today.getTime() + 86400000);
  const todayFollowups = leads.filter((lead) => lead.next_follow_up && new Date(lead.next_follow_up) >= today && new Date(lead.next_follow_up) < tomorrow);
  const todayTasks = tasks.filter((task) => task.due_at && new Date(task.due_at) >= today && new Date(task.due_at) < tomorrow && !isCompletedTask(task));
  const overdueLeads = leads.filter((lead) => lead.next_follow_up && new Date(lead.next_follow_up) < today && !isWon(lead.status) && String(lead.status).toLowerCase() !== 'lost');
  const wonLeads = leads.filter((lead) => isWon(lead.status));
  return { profile, leads, tasks, activities, notifications, assignedLeads: leads.length, todayFollowups: todayFollowups.length + todayTasks.length, wonLeads: wonLeads.length, overdueLeads: overdueLeads.length + tasks.filter(isOverdueTask).length, recentLeads: leads.slice(0, 6), followups: [...todayFollowups.map((lead) => ({ kind: 'lead', lead, title: lead.name, company: lead.company, time: lead.next_follow_up, status: lead.status })), ...todayTasks.map((task) => ({ kind: 'task', task, title: task.title, company: task.lead?.company || task.lead?.name || 'Task', time: task.due_at, status: task.type }))].sort((a,b) => new Date(a.time || 0) - new Date(b.time || 0)).slice(0, 6), unreadNotifications: notifications.filter((n) => !n.read_at).length, notifications };
}

export async function getReportsSummary({ from, to, mine = false } = {}) {
  const leads = mine ? await listMyLeads({ limit: 1000 }) : await listLeads({ limit: 1000 });
  const tasks = mine ? await listMyTasks({ from, to, limit: 1000 }) : await listTasks({ from, to, limit: 1000 });
  const won = leads.filter((lead) => isWon(lead.status));
  const lost = leads.filter((lead) => statusKey(lead.status) === 'Lost');
  const completed = tasks.filter(isCompletedTask);
  const overdue = tasks.filter(isOverdueTask);
  const revenueWon = won.reduce((sum, lead) => sum + Number(lead.value || 0), 0);
