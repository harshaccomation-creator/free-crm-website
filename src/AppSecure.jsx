...
function isProtected(path) {
  return path.startsWith('/employee') || path.startsWith('/admin') || path.startsWith('/super-admin') || path === '/leads' || path.startsWith('/leads/');
}
...
function isCompanyAdmin(role) { return role === 'company_admin'; }
...
export default function AppSecure() {
  ...
  useEffect(() => {
    if (auth.loading || !loggedIn || !protectedRoute) return;
    if (isSuperAdmin(role) && !path.startsWith('/super-admin')) navigate('/super-admin/dashboard', setPath);
    if (isCompanyAdmin(role) && (path.startsWith('/employee') || path.startsWith('/super-admin'))) navigate('/admin/dashboard', setPath);
    if (isEmployee(role) && (path.startsWith('/admin') || path.startsWith('/super-admin'))) navigate('/employee/dashboard', setPath);
  }, [auth.loading, loggedIn, protectedRoute, role, path]);
  ...
  if (loggedIn && path === '/') {
    ...
    if (home.startsWith('/super-admin')) return <SuperAdminDashboard />;
    if (home.startsWith('/admin')) return <AdminDashboard />;
