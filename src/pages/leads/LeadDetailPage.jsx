      ...
      <main className="lead-detail-ref-main">
        <section className="lead-detail-ref-hero">
          ...
          <div className="lead-detail-ref-actions">
            ...
            <button className="primary">⚱ Follow Up</button>
            <button>⋮</button>
          </div>
        </section>

        <nav className="lead-detail-ref-tabs">
          {['Overview','Activity','Tasks','Notes','Documents','Deals','Email History','WhatsApp History'].map((tab, index) => <button className={index === 0 ? 'active' : ''} key={tab}>{tab}</button>)}
        </nav>

        <section className="lead-detail-ref-grid">
          <div className="lead-detail-ref-left">
            <div className="lead-detail-ref-top">
              <article className="lead-detail-ref-card">
              ...
              <article className="lead-detail-ref-card lead-score-card">
                ...
                <button>View Score Details</button>
              </article>
            </div>
            ...
            <div className="lead-detail-ref-mid">
              <article className="lead-detail-ref-card summary-card">
                ...
                <p>He is interested in automation, reports and lead management.</p>
                <p>He requested pricing details and a demo.</p>
              </article>
              ...
              <article className="lead-detail-ref-card follow-card">
                ...
                <p>☎ Follow up call</p>
                <footer>Assigned to <span className="lead-ref-avatar">R</span> {lead.owner}<button>Mark as Done</button></footer>
              </article>
            </div>

            <article className="lead-detail-ref-card lead-deals-card">
              <header><h2>Deals (1)</h2><button>View All Deals</button></header>
              <table>
                <thead><tr><th>Deal Name</th><th>Deal Value</th><th>Stage</th><th>Expected Close Date</th><th>Status</th></tr></thead>
                <tbody><tr><td>CRM Software Deal</td><td>₹ 2,45,000</td><td><span className="lead-ref-pill purple">Proposal</span></td><td>{lead.expectedClose}</td><td><span className="lead-ref-pill green">Open</span></td></tr></tbody>
              </table>
            </article>
          </div>
          ...
          <aside className="lead-detail-ref-right">
            <article className="lead-detail-ref-card timeline-card">
              <header><h2>Lead Timeline</h2><select><option>All Activities</option></select></header>
              {['Lead created','Email opened','Phone call','Email sent','Note added','Task created'].map((item, index) => <div className="timeline-item" key={item}><span>{['♙','✉','☎','✉','☑','▣'][index]}</span><div><strong>{item}</strong><small>{index === 0 ? 'by ' + lead.owner : ['Product Brochure','Connected','Pricing Details','Interested in enterprise plan.','Follow up call'][index - 1]}<br />20 May 2025 {10 + index}:30 AM</small></div></div>)}
              <button>View All Activities</button>
            </article>
            <article className="lead-detail-ref-card tag-card"><h2>Tags</h2><div><span className="lead-ref-pill green">Interested</span><span className="lead-ref-pill blue">Budget Available</span><span className="lead-ref-pill purple">Quick Decision Maker</span><button>+ Add Tag</button></div></article>
          </aside>
        </section>
      </main>
    </div>
