  ...
  return (
    <div className="sf-dashboard lead-reference lead-detail-reference">
      ...
      <main className="lead-detail-ref-main">
        ...
        <nav className="lead-detail-ref-tabs">{['Overview','Activity','Tasks','Notes','Documents','Deals','Email History','WhatsApp History'].map((tab, index) => <button className={index === 0 ? 'active' : ''} key={tab}>{tab}</button>)}</nav>
        <section className="lead-detail-ref-grid">
          <div className="lead-detail-ref-left">
            ...
            <article className="lead-detail-ref-card lead-deals-card"><header><h2>Deals (1)</h2><button>View All Deals</button></header><table><thead><tr><th>Deal Name</th><th>Deal Value</th><th>Stage</th><th>Expected Close Date</th><th>Status</th></tr></thead><tbody><tr><td>CRM Software Deal</td><td>₹ 2,45,000</td><td><span className="lead-ref-pill purple">Proposal</span></td><td>{lead.expectedClose}</td><td><span className="lead-ref-pill green">Open</span></td></tr></tbody></table></article>
          </div>
