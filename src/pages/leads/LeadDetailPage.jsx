...
function ActivityTimeline({ items, onAdd }) {
  return <article className="ld-card ld-activity-panel"><header><h2>Activity Timeline</h2><div className="ld-activity-actions"><button type="button" className="ld-add-activity-btn" onClick={onAdd}>+ Add Activity</button><select><option>All Activities</option></select></div></header><div className="ld-activity-list">{items.map((item) => <div className={`ld-activity-row ${item.tone}`} key={item.title + item.time}><span className="ld-activity-icon"><Icon>{item.icon}</Icon></span><div className="ld-activity-text"><strong>{item.title}</strong><p>{item.text}</p></div><div className="ld-activity-meta">{item.time}<small>{item.user}</small></div></div>)}</div></article>;
}
function SideInfo() {
...
function TasksPanel({ tasks, onAdd, onComplete }) {
  ...
  return <section className="ld-card ld-tab-panel ld-tasks-panel"><header><div><h2>Tasks</h2><p>Follow-up/demo se task auto banega. Time cross hone par Overdue ho jayega.</p></div><button type="button" onClick={onAdd}>+ Add Task</button></header><div className="ld-task-stats"><span><b>{tasks.length}</b>Total</span><span className="danger"><b>{counts.danger}</b>Overdue</span><span><b>{counts.pending}</b>Pending</span><span className="done"><b>{counts.done}</b>Done</span></div>{tasks.map((task) => { const status = getTaskStatus(task); return <div className={`ld-task-row ${status.tone}`} key={task.id}><span className="ld-task-icon">{task.icon}</span><div><strong>{task.title}</strong><p>{task.source} • Due {formatDue(task.dueAt)}</p></div><b>{status.label}</b><button type="button" onClick={() => onComplete(task.id)}>{task.completed ? 'Done' : 'Complete'}</button></div>; })}</section>;
}
function SimplePanel({ title, action, rows, icon = '☷' }) {
  return <section className="ld-card ld-tab-panel"><header><h2>{title}</h2><button>{action}</button></header>{rows.map((row) => <div className="ld-tab-row" key={row[0]}><span className="ld-tab-dot blue"><Icon>{icon}</Icon></span><div><strong>{row[0]}</strong><p>{row[1]}</p></div><b>{row[2]}</b></div>)}</section>;
}
