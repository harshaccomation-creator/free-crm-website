...
function getLeadDate(lead) { if (lead.createdAt) return String(lead.createdAt).slice(0, 10); if (lead.createdDate) return String(lead.createdDate).slice(0, 10); if (lead.date) return String(lead.date).slice(0, 10); return todayISO(); }
...
export default function LeadListPage() {
  ...
  const [form, setForm] = useState({ ...emptyForm, owner: accessUser.name });
  const [errors, setErrors] = useState({});
  const [localLeads, setLocalLeads] = useState([]);

  const leads = useMemo(() => {
    const allRows = [...localLeads, ...empLeads].map((lead) => ({ ...lead, status: lead.status || "New", company: lead.company || "Not assign", source: lead.source || "Manual", value: lead.value || 0, score: scoreNumber(lead.score), createdDate: getLeadDate(lead) }));
    return filterRecordsForUser(allRows, accessUser);
  }, [localLeads, accessUser]);
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter((lead) => {
      ...
      const searchOk = !query || [lead.name, lead.company, lead.email, lead.phone, lead.source, lead.ownerName, lead.owner].join(" ").toLowerCase().includes(query);
  ...
  const updateForm = (field, value) => { setForm((old) => ({ ...old, [field]: value })); setErrors((old) => ({ ...old, [field]: "" })); };
  const openAddLead = () => { setForm({ ...emptyForm, owner: accessUser.name }); setErrors({}); setIsAddLeadOpen(true); };
  const closeAddLead = () => { setIsAddLeadOpen(false); setErrors({}); };
  ...
  const saveLead = (event) => {
    ...
    const nextErrors = validateLeadForm({ ...form, owner: accessUser.name });
    if (Object.keys(nextErrors).length) { setErrors(nextErrors); return; }
    const newLead = attachOwnerToRecord({
      id: `lead-${Date.now()}`,
      name: form.name.trim(),
    ...
    setAppliedFilters(emptyFilters); setTempFilters(emptyFilters); setSearch(""); setActiveTab("All"); setPage(1); closeAddLead();
  };
    ...
    <EmployeeShell>
      <div className="space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          <div><p className="text-xs font-bold text-orange-600 uppercase tracking-wider">Employee Workspace</p><h1 className="text-4xl font-black text-slate-900 tracking-tight">My Leads</h1><p className="text-lg text-slate-500 mt-2">Manage and track your assigned leads.</p></div>
