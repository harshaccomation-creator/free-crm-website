...
const sourceClass = { Website: 'blue', Referral: 'green', LinkedIn: 'purple', 'Cold Call': 'orange', 'Email Campaign': 'purple' };
    ...
    <div className="sf-dashboard lead-reference lead-list-reference">
      ...
      <main className="lead-ref-main">
        <header className="lead-ref-header">
          ...
          <div className="lead-ref-actions"><button>⇩ Export</button><button className="primary">＋ Add Lead</button></div>
        </header>
        ...
        <section className="lead-ref-table-card">
          <table className="lead-ref-table"><thead><tr><th><input className="lead-ref-check" type="checkbox" /></th><th>Lead Name</th><th>Company</th><th>Lead Source</th><th>Status</th><th>Last Activity</th><th>Next Follow Up</th><th>Lead Owner</th><th>Actions</th></tr></thead><tbody>{leads.map((lead) => <tr key={lead.id} onClick={() => openLead(lead.id)}><td><input className="lead-ref-check" type="checkbox" onClick={(event) => event.stopPropagation()} /></td><td><div className="lead-ref-person"><span className="lead-ref-avatar">{lead.initials}</span><div><strong>{lead.name}</strong><small>{lead.phone}</small></div></div></td><td>{lead.company}</td><td><span className={`lead-ref-pill ${sourceClass[lead.source] || 'blue'}`}>{lead.source}</span></td><td><span className={`lead-ref-pill ${statusClass[lead.status] || 'blue'}`}>{lead.status}</span></td><td>☎ {lead.lastActivity}</td><td>▣ {lead.nextFollowUp}</td><td><span className="lead-ref-owner"><span className="lead-ref-avatar">R</span>{lead.owner}</span></td><td>⋮</td></tr>)}</tbody></table>
          <footer className="lead-ref-table-footer"><span>Showing 1 to 6 of 45 leads</span><div className="lead-ref-pagination"><button>‹</button><button className="active">1</button><button>2</button><button>3</button><span>...</span><button>8</button><button>›</button><select><option>10 / page</option></select></div></footer>
        </section>
      </main>
    </div>
  );
