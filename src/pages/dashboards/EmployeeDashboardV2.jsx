    ...
    <div className="sf-dashboard employee-v2-page emp-mock-dashboard">
      ...
      <main className="employee-v2-main">
        ...
        <section className="employee-kpis">
          <button type="button" className="kpi-card blue" onClick={() => go('/leads')}><i>👥</i><div><span>Assigned Leads</span><strong>128</strong><Trend value="12%" text="from last week" /></div><svg viewBox="0 0 120 62"><path d="M5 52 C30 20 42 50 62 25 S93 8 115 18" /></svg></button>
          <button type="button" className="kpi-card purple" onClick={() => go('/employee/tasks')}><i>▣</i><div><span>Today Follow-ups</span><strong>24</strong><Trend value="8%" text="from yesterday" /></div><svg viewBox="0 0 120 62"><path d="M5 50 C28 30 45 44 60 26 S92 12 115 18" /></svg></button>
          <button type="button" className="kpi-card green" onClick={() => go('/employee/won')}><i>🏆</i><div><span>Won Leads</span><strong>23</strong><Trend value="27%" text="from last month" /></div><svg viewBox="0 0 120 62"><path d="M5 55 C28 30 44 42 64 30 S90 4 116 14" /></svg></button>
          <button type="button" className="kpi-card orange" onClick={() => go('/employee/tasks')}><i>◷</i><div><span>Overdue Leads</span><strong>18</strong><Trend direction="down" value="5" text="from yesterday" /></div><svg viewBox="0 0 120 62"><path d="M5 52 C26 25 43 40 62 22 S92 10 115 14" /></svg></button>
        </section>

        <section className="employee-main-grid">
          <article className="emp-card followup-card"><div className="emp-card-head"><h2><span>▣</span> Today&apos;s Follow-ups</h2><button type="button" onClick={() => go('/employee/calendar')}>View All ›</button></div><div className="followup-list">{visibleFollowups.map(([avatar, name, company, time, status]) => <div className="followup-row" key={name}><span className="avatar-badge">{avatar}</span><div><strong>{name}</strong><small>{company}</small></div><time>◷ {time}</time><em className={`status ${statusClass(status)}`}>• {status}</em><button type="button" onClick={() => toast(`Call action for ${name}`)}>📞 Call</button><button type="button" className="wa" onClick={() => toast(`WhatsApp action for ${name}`)}>☘ WhatsApp</button></div>)}</div></article>
