...
export default function ContactsPage({ role = 'employee' }) {
  ...
  return (
    <SaasLayout role={role} title="Contacts" subtitle="All real customer contacts generated from CRM leads and assigned records." actions={<button className="saas-btn primary" onClick={() => goTo('/leads')}>+ Add from Lead</button>}>
      ...
      <section className="saas-card saas-table-card"><div className="saas-table-wrap"><table className="saas-table"><thead><tr><th>Contact</th><th>Company</th><th>Source</th><th>Status</th><th>Owner</th><th>Last Activity</th></tr></thead><tbody>{rows.map((contact) => <tr key={contact.id} onClick={() => goTo(`/leads/${contact.id}`)}><td className="saas-title-cell"><strong>{contact.name}</strong><small>{contact.email} • {contact.phone}</small></td><td>{contact.company}</td><td><span className="saas-badge">{contact.source}</span></td><td><span className="saas-badge green">{contact.status}</span></td><td>{contact.owner}</td><td>{fmt(contact.lastActivity)}</td></tr>)}</tbody></table></div>{!loading && !rows.length ? <SaasEmpty title="No contacts found" text="Contacts are created from real leads. Add or assign leads to populate this module." /> : <footer className="saas-footer"><span>Showing {rows.length} contacts</span><button className="saas-btn" onClick={load}>Refresh</button></footer>}</section>
    </SaasLayout>
  );
}
