  ...
  return (
    <div className="landing-page">
      <div className="top-mini-bar">
        <div className="landing-shell mini-inner">
          <button onClick={() => action('English selected')}>🌐 English</button>
          <button onClick={() => action('High contrast ready')}>◐ High Contrast</button>
          <span />
          <button onClick={() => setModal('Support')}>🎧 Customer Support</button>
          <button onClick={() => setModal('Contact Sales')}>✉ Contact Sales</button>
      ...
      <header className="main-header">
        <div className="landing-shell header-inner">
          <button className="brand-wrap" onClick={() => goTo('hero')}>
            <span className="brand-mark">S</span>
            <span className="brand-name">Sales<span>Flow</span></span>
          </button>
          <nav className="desktop-menu">
            {menuItems.map((item) => <button key={item} onClick={() => goTo(item.toLowerCase())}>{item}</button>)}
          </nav>
          <div className="header-actions">
            <button className="btn btn-soft" onClick={() => setModal('Book a Demo')}><span className="btn-icon">📅</span>Book a Demo</button>
            <button className="btn btn-primary" onClick={openLoginPage}><span className="btn-icon">🚀</span>Start 7-Day Free Trial</button>
          </div>
        </div>
      </header>
      ...
      <main id="hero" className="hero-section">
        <div className="landing-shell hero-layout">
          <section className="hero-content">
            <span className="pill">⚡ Lead follow-up & sales team CRM</span>
            <h1>Never miss a sales <span>follow-up</span> again.</h1>
            <p>SalesFlow Hub helps teams manage leads, follow-ups, employee activity, payments and reports in one simple CRM.</p>

            <div className="value-grid">
              <button onClick={() => action('Lead capture selected')}><strong>◎ Capture More Leads</strong><span>Collect leads from every channel in one clean pipeline.</span></button>
            ...
            <div className="hero-cta">
              <button className="btn btn-primary btn-big" onClick={openLoginPage}><span className="btn-icon">🚀</span>Start 7-Day Free Trial</button>
              <button className="btn btn-outline btn-big" onClick={() => goTo('platform-highlights')}><span className="btn-icon">▶</span>Watch Demo Video</button>
            </div>
            <p className="hero-trial-note">Create your account and get instant access to a 7-day free trial.</p>
          </section>

          <section className="dashboard-mock">
            <div className="mock-sidebar">
              <strong>SalesFlow</strong>
              {sideItems.map(([item, icon]) => <button key={item} className={item === 'Dashboard' ? 'active' : ''} onClick={() => action(item + ' preview opened')}><span>{icon}</span>{item}</button>)}
            </div>
            <div className="mock-main">
              <div className="mock-top">
                <div><span>Good morning, Aman</span><h3>Sales Overview</h3></div>
                <button onClick={() => action('Date filter opened')}>📅 May 1 - May 31</button>
              </div>
              <div className="mock-stats">
                {stats.map((item) => <button key={item.label} onClick={() => action(item.label + ': ' + item.value)}><span>{item.label}</span><strong>{item.value}</strong><small>{item.trend}</small></button>)}
              </div>
              <div className="mock-card chart-card premium-preview-card">
                <div className="card-head"><span>Weekly Performance</span><button onClick={() => action('Report opened')}>📊 View Report</button></div>
                <div className="preview-card-grid">
                  <div className="mini-chart-panel">
                    <div className="mini-chart-bars"><i style={{ height: '36%' }} /><i style={{ height: '58%' }} /><i style={{ height: '46%' }} /><i style={{ height: '74%' }} /><i style={{ height: '62%' }} /><i style={{ height: '88%' }} /></div>
                    <div className="mini-kpis"><span><b>24</b> Calls</span><span><b>12</b> Follow-ups</span><span><b>5</b> Deals</span></div>
                  </div>
                  <div className="inline-task-panel">
                    <strong>Upcoming Tasks</strong>
                    <button onClick={() => action('Call task opened')}>☎ Call with new lead</button>
                    <button onClick={() => action('Follow-up opened')}>↻ Follow-up with Raj</button>
                    <button onClick={() => action('Proposal opened')}>✉ Send proposal</button>
                  </div>
                </div>
              </div>
            </div>
