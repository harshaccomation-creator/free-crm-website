...
const issueTypes = ["Lead Issue", "Follow-up Issue", "Task Issue", "Payment Issue", "Report Issue", "Technical Bug", "Feature Request", "Other"];
...
export default function SupportPage() {
  const [form, setForm] = useState({ category: "Lead Issue", priority: "Medium", subject: "", message: "" });
  ...
  const submitTicket = async (event) => {
    ...
    setForm({ category: "Lead Issue", priority: "Medium", subject: "", message: "" });
    setLoading(false);
  };
    ...
    <EmployeeShell>
      <div className="space-y-5">
        <section className="rounded-3xl bg-slate-950 text-white p-6 shadow-xl">
          ...
          <h1 className="mt-2 text-3xl font-black tracking-tight">CRM Help & Support</h1>
          <p className="mt-2 max-w-2xl text-sm font-medium text-slate-300">Logged-in users ke liye CRM related issues: leads, follow-ups, tasks, reports, payments aur technical bugs.</p>
        </section>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
            ...
            <div><h2 className="text-lg font-black text-slate-900">Create CRM Ticket</h2><p className="text-sm text-slate-500">Login/OTP issue ke liye login page par public support use karo.</p></div>
          </div>
          ...
          <form className="mt-5 space-y-4" onSubmit={submitTicket}>
            ...
            <label className="block space-y-2"><span className="text-xs font-black uppercase tracking-wider text-slate-500">Subject</span><input value={form.subject} onChange={(e) => update("subject", e.target.value)} placeholder="Example: Follow-up reminder not showing" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-orange-200" /></label>
            <label className="block space-y-2"><span className="text-xs font-black uppercase tracking-wider text-slate-500">Message</span><textarea value={form.message} onChange={(e) => update("message", e.target.value)} rows={6} placeholder="Issue kaunsa page par aa raha hai, kya error hai, aur kab se aa raha hai..." className="w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-800 outline-none focus:ring-2 focus:ring-orange-200" /></label>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><h2 className="text-lg font-black text-slate-900">My CRM Tickets</h2><p className="text-sm text-slate-500">Latest in-app support requests.</p></div><TicketCheck className="h-5 w-5 text-orange-500" /></div>
