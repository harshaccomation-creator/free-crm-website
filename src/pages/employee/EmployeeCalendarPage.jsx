    ...
    <EmployeeShell>
      <div className="space-y-5">
        <div className="flex items-center justify-between gap-4">
          ...
          <button onClick={() => setIsScheduleOpen(true)} className="inline-flex items-center gap-2 px-4 h-10 rounded-xl bg-orange-500 text-white text-sm font-bold shadow-lg shadow-orange-500/20">
          ...
          <aside className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100">
              ...
              <p className="text-sm text-slate-500 mt-1">
                {selectedEvents.length ? `${selectedEvents.length} task/event on this date.` : "Next scheduled events."}
              </p>
            </div>
              ...
              {(selectedEvents.length ? selectedEvents : upcomingEvents).map((item) => {
                ...
                return (
                  <button key={item.id} type="button" onClick={() => goLead(item.leadId)} className="w-full px-5 py-4 flex items-start gap-3 text-left hover:bg-orange-50/70 transition-colors">
                    <div className="w-10 h-10 rounded-xl bg-orange-50 text-orange-600 grid place-items-center shrink-0">
