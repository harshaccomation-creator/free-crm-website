...
function buildVisibleTasks(state, accessUser) { return filterRecordsForUser(workflowTasksToPageTasks(state), accessUser); }
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-200 space-y-4">
            <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4"><div><h2 className="text-xl font-black text-slate-900">All Tasks</h2><p className="text-sm text-slate-500 mt-1">Showing {filteredTasks.length} of {tasks.length} real assigned tasks.</p></div><button type="button" onClick={resetFilters} className="h-10 px-4 rounded-xl border border-slate-200 text-slate-700 font-black text-sm hover:bg-slate-50">Reset Filter</button></div>
