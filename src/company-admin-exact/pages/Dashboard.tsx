...
export default function Dashboard() {
  const statCards = [
    { title: "Total Leads", value: dashboardStats.totalLeads, icon: Users, color: "text-blue-600", trend: "+12%" },
    { title: "Active Leads", value: dashboardStats.activeLeads, icon: UserCheck, color: "text-purple-600", trend: "+5%" },
    { title: "Today Follow-ups", value: dashboardStats.todayFollowUps, icon: PhoneCall, color: "text-blue-500", trend: "-2%" },
    { title: "Overdue Leads", value: dashboardStats.overdueLeads, icon: AlertCircle, color: "text-orange-500", trend: "+8%" },
    { title: "Won Leads", value: dashboardStats.wonLeads, icon: CheckCircle2, color: "text-green-600", trend: "+15%" },
    { title: "Lost Leads", value: dashboardStats.lostLeads, icon: XCircle, color: "text-red-500", trend: "-4%" },
    { title: "Total Payments", value: dashboardStats.totalPayments, icon: CreditCard, color: "text-indigo-600", trend: "+20%" },
    { title: "Pending Tasks", value: dashboardStats.pendingTasks, icon: CheckSquare, color: "text-amber-500", trend: "-1%" },
    { title: "Team Members", value: dashboardStats.teamMembers, icon: Users2, color: "text-slate-600", trend: "0%" },
  ];
          ...
          <Card key={i} className="rounded-xl border-gray-100 shadow-sm" data-testid={`card-stat-${stat.title.toLowerCase().replace(" ", "-")}`}>
            <CardContent className="p-6">
              ...
              <div className="mt-4 flex items-center text-xs">
                <span className={stat.trend.startsWith("+") ? "text-green-600 font-medium" : stat.trend === "0%" ? "text-slate-500 font-medium" : "text-red-600 font-medium"}>
                  {stat.trend}
                </span>
        ...
        <Card className="shadow-sm border-gray-100">
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-gray-100">
              {followups.filter(f => f.status === 'Overdue').slice(0, 4).map((f) => (
            ...
            <div className="space-y-4">
              {teamMembers.slice(0, 3).map((member, i) => (
                <div key={member.id} className="flex items-center justify-between p-3 border border-gray-100 rounded-lg hover:border-gray-200 transition-colors">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-green-600">₹{member.wonLeads * 1.5}L</p>
                  </div>
                </div>
              ))}
            </div>
